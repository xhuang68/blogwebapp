...
env.user = 'root'
env.sudo_user = 'root'
env.hosts = ['104.236.176.102']

db_user = 'www-data'
db_password = 'www-data'

_TAR_FILE = 'dist-blogwebapp.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/blogwebapp'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-blogwebapp-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick blogwebapp > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop blogwebapp')
        sudo('supervisorctl start blogwebapp')
...
def restore2local():
    ...
    sqls = [
        ...
        'create database blogwebapp;',
        'grant select, insert, update, delete on blogwebapp.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s blogwebapp < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
